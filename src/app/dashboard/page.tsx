                  ...
                  <div className="px-6 pb-4 ml-20 space-y-3 text-sm">
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-gray-300">{booking.booked_via === 'ai_call' ? 'AI Phone Call' : 'Manual Entry'}</p>
                      </div>
                    </div>
                    ...
                    {booking.dietary_notes && (
                      <div className="bg-amber-500/10 rounded-lg p-3 border border-amber-500/20">
                        <div className="flex items-center gap-2 mb-1">
                          <svg className="w-4 h-4 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15.75h.007v.008H12v-.008z" /></svg>
                          <span className="text-amber-400 text-xs font-semibold uppercase tracking-wider">Special Requests</span>
                        </div>
                        <p className="text-amber-100 text-sm font-medium">{booking.dietary_notes}</p>
                      </div>
                    )}
                    ...
                    {booking.booked_via === 'ai_call' && (
                      <div className="bg-gray-800/50 rounded-lg p-3 border border-gray-700/50">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="flex items-center gap-3">
                            <button
                              onClick={e => { e.stopPropagation(); setShowTranscript(showTranscript === booking.id ? null : booking.id) }}
                            ...
                            >
                              ...
                              {showTranscript === booking.id ? 'Hide transcript' : 'View transcript'}
                            </button>
                        ...
                        {/* Transcript */}
                        {showTranscript === booking.id && callLogs[booking.id]?.transcript && (
                          <div className="mt-3 pt-3 border-t border-gray-700/50">
                            <p className="text-gray-500 text-xs font-medium mb-2 uppercase tracking-wider">Full Transcript</p>
                            <div className="bg-gray-900/80 rounded-lg p-3 max-h-64 overflow-y-auto text-xs text-gray-400 leading-relaxed whitespace-pre-wrap font-mono">
                              {callLogs[booking.id]!.transcript}
                            </div>
                          </div>
                        )}
                      </div>
